...
// packages/adapters/acc/dist/acc-adapter.js
...
var AccAdapter = class {
  socket;
  lastFrame = null;
  port;
  pythonProcess = null;
  constructor(port = 9300) {
    this.port = port;
  ...
  spawnBridge() {
    const scriptPath = import_path.default.resolve(__dirname, "../../python/acc_bridge.py");
  ...
  mapToTelemetryFrame(raw) {
    ...
    return {
      ...
      physics: {
        ...
        // normalized or degrees? Python should normalize
        lateralG: raw.physics.camberRAD?.[0] || 0,
        // Placeholder
      ...
      temps: {
        ...
        // [FL, FR, RL, RR]
        brakeC: []
        // Not available in Shared Memory v1
      },
      suspension: {
...
var adapter = new AccAdapter(9300);
adapter.start((frame) => {
  const data = {
    ...
    steering_rad: frame.physics.steeringAngle,
    // Ensure units match
    temps: {
